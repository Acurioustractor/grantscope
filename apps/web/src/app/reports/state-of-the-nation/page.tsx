...
const SNAPSHOT_VERIFIED_AT = '29 April 2026';

const SNAPSHOT_STATS = {
  entityCount: 591761,
  relationshipCount: 1526943,
  charityCount: 64988,
  foundationCount: 10918,
  grantCount: 32018,
  oricCount: 7369,
  oricActiveCount: 3366,
  programCount: 3296,
  austenderCount: 796701,
  asicCount: 491000,
  atoCount: 26241,
  asxCount: 1976,
  oricAcncMatch: 1510,
  sourceCount: 53431,
  oricByState: [] as Array<[string, number]>,
  atoTop: [] as Array<{
    entity_name: string;
    abn: string | null;
...
function fmtApprox(n: number) { return `~${n.toLocaleString()}`; }
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <div className="text-xs font-black text-bauhaus-red mt-4 mb-1 uppercase tracking-widest">Snapshot Report</div>
        <h1 className="text-3xl sm:text-4xl font-black text-bauhaus-black mb-3">
          The State of Business, Power and Money in Australia
        </h1>
        <p className="text-bauhaus-muted text-base sm:text-lg max-w-3xl leading-relaxed font-medium">
          Snapshot figures are cached so the report opens instantly; source tables refresh through the data pipeline.{' '}
          {fmt(s.entityCount)} resolved entities. {fmt(s.relationshipCount)} cross-system relationships.{' '}
          {fmt(s.charityCount)} charities. {fmtApprox(s.asicCount)} ASIC company records. {fmt(s.oricCount)} ORIC corporations.{' '}
          {fmtApprox(s.austenderCount)} government contracts. {fmt(s.atoCount)} tax transparency records.
        </p>
        <p className="mt-3 text-[11px] font-black uppercase tracking-[0.22em] text-bauhaus-muted">
          {SNAPSHOT_LABEL} · checked {SNAPSHOT_VERIFIED_AT} · large graph tables use cached catalog statistics where exact scans exceed runtime limits
        </p>
      </div>
      ...
      {/* Live Data Grid */}
      <section className="mb-12">
        <h2 className="text-xl font-black text-bauhaus-black mb-6 uppercase tracking-widest">What We&apos;ve Mapped — Current Snapshot</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-0">
          {[
            { label: 'Resolved Entities', value: fmt(s.entityCount), color: 'bg-bauhaus-red' },
            { label: 'Graph Links', value: fmt(s.relationshipCount), color: 'bg-bauhaus-blue' },
            { label: 'Charities (ACNC)', value: fmt(s.charityCount), color: 'bg-bauhaus-red' },
            { label: 'Companies (ASIC)', value: fmtApprox(s.asicCount), color: 'bg-bauhaus-blue' },
            ...
            { label: 'Govt Contracts', value: fmtApprox(s.austenderCount), color: 'bg-bauhaus-black' },
            ...
            { label: 'Source Pages', value: fmtApprox(s.sourceCount), color: 'bg-bauhaus-black' },
          ].map((stat) => (
      ...
      <section className="mb-12">
        ...
        <div className="space-y-0">
          {[
            { layer: '1', title: 'Entity Registry', desc: 'Resolved entities across business, charity, Indigenous corporation, foundation, ASX, and related registers. Connected by ABN where available.', status: `${fmt(s.entityCount)} entities`, color: 'bg-bauhaus-red', live: true },
            { layer: '2', title: 'Government Procurement', desc: 'Federal contracts from AusTender. Who wins government money, and how much.', status: `${fmtApprox(s.austenderCount)} contracts`, color: 'bg-bauhaus-blue', live: s.austenderCount > 0 },
      ...
      <section className="mb-8">
        <div className="border-4 border-bauhaus-black p-6 bg-gray-50 font-mono text-sm">
          <div className="font-black text-bauhaus-black mb-3 uppercase tracking-widest text-xs not-italic font-sans">Cached Data Snapshot</div>
          <pre className="text-bauhaus-black whitespace-pre-wrap">{`SNAPSHOT STATUS
├── Label                      ${SNAPSHOT_LABEL}
├── Checked                    ${SNAPSHOT_VERIFIED_AT}
└── Runtime note               large graph counts are cached so the report opens instantly

GRAPH SCALE
├── Resolved entities          ${fmt(s.entityCount).padStart(10)}
└── Cross-system relationships ${fmt(s.relationshipCount).padStart(10)}

SOURCE TABLES
├── Charities (ACNC)           ${fmt(s.charityCount).padStart(10)}
├── Companies (ASIC index)    ${fmtApprox(s.asicCount).padStart(10)}
├── Foundations                ${fmt(s.foundationCount).padStart(10)}
├── Indigenous Corps (ORIC)    ${fmt(s.oricCount).padStart(10)}  (${fmt(s.oricActiveCount)} registered)
├── ASX Listed                 ${fmt(s.asxCount).padStart(10)}
├── Grant Opportunities        ${fmt(s.grantCount).padStart(10)}
├── Source Frontier Pages     ${fmtApprox(s.sourceCount).padStart(10)}
├── Govt Contracts            ${fmtApprox(s.austenderCount).padStart(10)}
