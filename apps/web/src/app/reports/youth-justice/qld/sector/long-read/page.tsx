    ...
    <div>
      ...
      <ReportSection id="summary" kicker="Executive Summary" title="14 findings · what the data says">
        ...
        <Finding n={10} title="Same providers hold federal procurement, state grants, and foundation grants" severity="warn">
          <p>
            Federal AusTender contracts to YJ-relevant suppliers in CivicGraph total <span className="font-black">{money(r.contracts.reduce((s, c) => s + c.total, 0))}</span> across <span className="font-black">{r.contracts.reduce((s, c) => s + c.contracts, 0)}</span> contracts &mdash; concentrated among the same {r.contracts.length} suppliers that hold the largest state-budget grants. Single-provider concentration carries operational risk (failure of one provider has cascading impact) and bargaining-asymmetry risk (commissioners face few real alternatives). <SourceLink href="#src-austender">[7]</SourceLink>
          </p>
        </Finding>
      ...
      <ReportSection id="volume-2" kicker="Volume 2" title="The Funnel — cross-system pathways">
        <p>
          Children don&apos;t arrive in custody directly. They arrive through pipelines &mdash; child protection, education disengagement, family violence, disability, mental health, substance use. Every system that fails them is upstream of the watchhouse.
        </p>
        ...
        <p>
          QLD has approximately <span className="font-black">{fmt(r.ndisYouth)}</span> NDIS participants in the 15–18 age band (state-wide aggregate). Significant numbers cluster in the same service districts as the LGA youth-offender-rate hotspots &mdash; Townsville, Logan, Mount Isa. Therapeutic, culturally-safe, NDIS-eligible programs in these districts are scarce; the structural gap is not eligibility but provider capacity. <SourceLink href="#src-ndis">[8]</SourceLink>
        </p>
      ...
      <ReportSection id="volume-3" kicker="Volume 3" title="The Money">
        ...
        <p>
          Australian foundations index in CivicGraph at significant annual scales: Paul Ramsay Foundation, Minderoo, BHP Foundation, Smith Family Foundation, Macquarie Group Foundation. Combined annual giving across the top 10 indexed in our dataset: substantial &mdash; but the thematic addresses (youth, indigenous, education, child welfare) rarely route to QLD youth-justice specifically. Foundation strategy has so far preferred adjacent thematic areas (early-childhood education, employment, trauma-informed services for women and children) over direct youth-justice anchoring. <SourceLink href="#src-pmra">[24]</SourceLink> <SourceLink href="#src-minderoo">[25]</SourceLink>
        </p>
      ...
      <ReportSection id="volume-4" kicker="Volume 4" title="The Network">
        <p>
          A handful of organisations sit at the centre of the system. They hold contracts across multiple topic streams, span multiple states, and overlap at the governance level &mdash; same directors, same auditors, same board interlocks. This isn&apos;t scandalous; it&apos;s structural. But it shapes who gets heard in policy, who has the capacity to absorb a growth budget, and where bargaining power sits when contracts are renegotiated.
        </p>
      ...
      <ReportSection id="volume-5" kicker="Volume 5" title="The Evidence">
        ...
        <p>
          Evidence levels split roughly into three tiers: <span className="font-black">Proven</span> (rigorously evaluated, demonstrable outcomes), <span className="font-black">Promising</span> (community-endorsed, emerging evidence), and <span className="font-black">Untested</span> (theory or pilot stage). Most ALMA-listed QLD interventions sit at &ldquo;Promising&rdquo; &mdash; reflecting not that programs don&apos;t work, but that evaluation funding hasn&apos;t historically been resourced alongside delivery.
        </p>

        <h3 className="text-lg font-black uppercase tracking-tight text-bauhaus-black mt-8 mb-3">The unfunded effective programs</h3>
        <p>
          {r.unfunded.length > 0 ? (
            <>CivicGraph identifies <span className="font-black">{r.unfunded.length}</span> ALMA-listed QLD or national programs graded &ldquo;Proven&rdquo; or &ldquo;Promising&rdquo; with no traceable funding link in our dataset. Examples include: {r.unfunded.slice(0, 4).map((u, i) => (<span key={i}><span className="font-black">{u.name}</span>{i < Math.min(3, r.unfunded.length - 1) ? ', ' : ''}</span>))}. These programs are running &mdash; mostly with grant top-ups, philanthropic seed funding, or volunteer labour &mdash; but have no operational state-budget anchor.</>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><span className="font-black">Royal Commission into Aboriginal Deaths in Custody (1991)</span> &mdash; 339 recommendations, many directly relevant to youth-justice over-representation. <SourceLink href="#src-aboriginal-deaths">[17]</SourceLink></li>
          <li><span className="font-black">Bringing Them Home (1997)</span> &mdash; the structural relationship between forced child removal and intergenerational harm runs directly into the contemporary YJ system. <SourceLink href="#src-bringing-them-home">[19]</SourceLink></li>
          <li><span className="font-black">Pathways to Justice (ALRC, 2018)</span> &mdash; specific recommendations on ACCO funding, justice reinvestment, and bail-and-remand reform. <SourceLink href="#src-pathways">[18]</SourceLink></li>
          <li><span className="font-black">NT Royal Commission (2017)</span> &mdash; 227 recommendations covering detention, child protection, policing, and community-led prevention. Structurally applicable beyond the NT. Implementation has been partial. <SourceLink href="#src-nt-rc">[16]</SourceLink></li>
        ...
        <p>
          The pattern is consistent: the prescriptive content of these reports overlaps significantly. The implementation gap is the recurring failure mode &mdash; not the absence of recommendations.
        </p>
      </ReportSection>
      ...
      <ReportSection id="volume-6" kicker="Volume 6" title="The Place">
        <p>
          Youth justice doesn&apos;t happen statewide; it happens in places. The same handful of QLD LGAs &mdash; Townsville, Logan, Mount Isa, Cherbourg, Cairns &mdash; appear at the top of every cross-system metric: youth-offender rate, NDIS participant density, low-ICSEA schools, JobSeeker recipients, foster-care entries.
        </p>
      ...
      <ReportSection id="volume-7" kicker="Volume 7" title="Policy &amp; capacity signals">
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><span className="font-black">&ldquo;Detention as a last resort&rdquo; removed</span> from the Youth Justice Act &mdash; reversing the foundational sentencing principle that had distinguished youth from adult criminal law since 1992.</li>
          <li><span className="font-black">Adult sentences for 13 listed offences</span> committed by children &mdash; murder, manslaughter, robbery, dangerous operation of a vehicle and others. Same maximum, mandatory and minimum penalties as adults.</li>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><span className="font-black">Cleveland Youth Detention Centre (27 Aug 2024)</span> &mdash; chronic staff shortages, children locked alone in their rooms. On one inspection day, 40% of Cleveland&apos;s 96 inmates were held in bare cells. Average separation length in 2022&ndash;23 was 8 hrs 36 min. 15 recommendations.</li>
          <li><span className="font-black">Cairns + Murgon watchhouses (2024)</span> &mdash; specific focus on children in regional watchhouses, the operational reality of the watchhouse-as-overflow pattern in §1.</li>
      ...
      <ReportSection id="shift" kicker="Closing" title="What would shift this">
        <p>
          Three structural moves stand out from the data. None require new policy frameworks &mdash; the prescriptive content of Closing the Gap, Pathways to Justice, the Justice Reinvestment Network&apos;s sector roadmap, and the QLD government&apos;s own Youth Justice Strategy 2019–2023 already point in this direction.
        </p>
      ...
      <section className="border-4 border-bauhaus-black p-8 bg-bauhaus-yellow mb-12 mt-12">
        ...
        <p className="text-bauhaus-black font-medium leading-relaxed max-w-3xl mb-5">
          We&apos;re building CivicGraph in public. Same pipeline works for any sector, peak body, or place in Australia. Tell us what you found valuable and what you&apos;d want next &mdash; anonymously if you like.
        </p>
