...
async function getStateReport(stateCode: string) {
 ...
 safe(supabase.rpc('exec_sql', { query: `SELECT sitting_date::text, speaker_name, speaker_party, subject, substring(body_text, 1, 280) AS snippet, source_url FROM public.civic_hansard WHERE jurisdiction = '${stateCode}' AND length(body_text) > 100 AND speaker_name IS NOT NULL AND speaker_name NOT ILIKE '%Speaker%' AND speaker_name NOT ILIKE '%Hansard%' AND (body_text ~* '(youth justice|adult crime|youth detention|watchhouse|breach of bail|young offender)') ORDER BY sitting_date DESC NULLS LAST LIMIT 8` }), 'reports/youth-justice/[state]/sector') as Promise<HansardRow[] | null>,
 safe(supabase.rpc('exec_sql', { query: `SELECT published_at::text, minister_name, portfolio, headline, source_url FROM public.civic_ministerial_statements WHERE jurisdiction = '${stateCode}' AND (topics @> ARRAY['youth-justice'] OR headline ~* '(youth|detention|watch.?house|adult crime|bail|young offender)') AND published_at > NOW() - INTERVAL '24 months' ORDER BY published_at DESC LIMIT 8` }), 'reports/youth-justice/[state]/sector') as Promise<MinStatement[] | null>,
 safe(supabase.rpc('exec_sql', { query: `SELECT count(*)::int AS c, SUM(contract_value)::bigint AS total FROM public.austender_contracts WHERE supplier_name ILIKE ANY (ARRAY['%youth justice%','%PCYC%','%mission australia%','%lifeline%','%anglicare%','%uniting%','%save the children%']) AND contract_value > 0` }), 'reports/youth-justice/[state]/sector') as Promise<Array<{ c: number; total: number }> | null>,
 safe(supabase.rpc('exec_sql', { query: `SELECT count(*)::int AS c FROM public.alma_interventions_valid WHERE ('${stateCode}' = ANY(geography) OR EXISTS (SELECT 1 FROM unnest(geography) g WHERE g ILIKE '%${stateCode}%'))` }), 'reports/youth-justice/[state]/sector') as Promise<Array<{ c: number }> | null>,
 safe(supabase.rpc('exec_sql', { query: `SELECT bill_name, sponsor, sponsor_party, introduced_date::text, status, status_date::text, source_url, explanatory_note_url, statement_of_compatibility_url FROM public.parliament_bills WHERE jurisdiction = '${stateCode}' AND is_yj_relevant = true ORDER BY status_date DESC NULLS LAST, introduced_date DESC NULLS LAST LIMIT 10` }), 'reports/youth-justice/[state]/sector') as Promise<StateBill[] | null>,
 ]);
...
export default async function StateYjSectorPage({ params }: { params: Promise<{ state: string }> }) {
 ...
 Same investigative pattern as the <Link href="/reports/youth-justice/qld/sector" className="text-bauhaus-blue font-black hover:underline">QLD flagship</Link>, applied to {meta.label}. Live data only, detention-vs-community spend, top funded recipients, cross-sector providers, ALMA evidence base, and parliamentary signals. State-specific curated content (planned facilities, legislation timeline, oversight findings) is in research; <Link href="/feedback?subject=yj-state-deepdive" className="text-bauhaus-blue font-black hover:underline">tell us</Link> what to prioritise next.
 </p>
 </div>
