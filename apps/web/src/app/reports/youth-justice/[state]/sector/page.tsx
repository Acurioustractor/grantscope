...
async function getStateReport(stateCode: string) {
 ...
 safe(supabase.rpc('exec_sql', { query: `SELECT count(*)::int AS c FROM public.alma_interventions WHERE ('${stateCode}' = ANY(geography) OR EXISTS (SELECT 1 FROM unnest(geography) g WHERE g ILIKE '%${stateCode}%'))` }), 'reports/youth-justice/[state]/sector') as Promise<Array<{ c: number }> | null>,
