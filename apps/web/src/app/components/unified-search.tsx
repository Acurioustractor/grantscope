...
import { SearchResultList, flatResults, toSearchResults, type SearchResults } from './search-results';
...
export function UnifiedSearch() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      fetch(`/api/global-search?q=${encodeURIComponent(query)}`, { signal: controller.signal })
        .then(res => res.json())
        .then(data => {
          setSearchResults(toSearchResults(data));
          setSelectedIndex(0);
          setLoading(false);
        })
        .catch(err => {
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    const allResults = searchResults ? flatResults(searchResults) : [];
    ...
    <div className="mb-10 max-w-2xl">
      ...
      {hasSearchResults && mode === 'search' && (
        <div className="mt-4 border-4 border-bauhaus-black bg-white max-h-[50vh] overflow-y-auto">
          <SearchResultList results={searchResults!} selectedIndex={selectedIndex} onSelect={(href) => router.push(href)} />
        </div>
      )}
