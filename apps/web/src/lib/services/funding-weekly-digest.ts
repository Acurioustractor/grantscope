...
export interface FundingDigestMetrics { applyNow: number; quarantined: number; queueSize: number; decisionsThisWeek: number; pursuedTotal: number; submittedTotal: number; wonTotal: number; historicalWins: number; pursueToSubmissionRate: number | null; medianDecisionHours: number | null; decisionReadyProfiles: number; activeProfiles: number; failingSources: number; queueWithinLimit: boolean; unsupportedInQueue: number; benchmarkReviewedThisWeek: number; benchmarkWeeklyRemaining: number; benchmarkBalancedProjects: number }
...
export function buildPriorityActions(metrics: FundingDigestMetrics): string[] { const actions: string[] = []; if (metrics.queueSize > 0) actions.push(`Resolve the ${Math.min(metrics.queueSize, 5)} funding decisions in this week's bounded queue.`); if (metrics.benchmarkWeeklyRemaining > 0) actions.push(`Review ${metrics.benchmarkWeeklyRemaining} remaining benchmark cases to improve project matching without exceeding the weekly learning cap.`); if (metrics.decisionReadyProfiles < metrics.activeProfiles) actions.push(`Complete remaining applicant-route, geography, funding-block and evidence facts for ${metrics.activeProfiles - metrics.decisionReadyProfiles} project profiles.`); if (metrics.failingSources > 0) actions.push(`Repair or review ${metrics.failingSources} funding source failures recorded this week.`); if (metrics.unsupportedInQueue > 0) actions.push(`Remove ${metrics.unsupportedInQueue} unsupported opportunities from the decision queue.`); if (!actions.length) actions.push('Review outcomes and refresh the next five funding decisions.'); return actions.slice(0, 5); }
...
export async function generateFundingWeeklyDigest(slug = 'act'): Promise<FundingWeeklyDigest> {
  ...
  const projectsResult = await db.from('org_projects').select('code').eq('org_profile_id', org.id).eq('status', 'active');
  if (projectsResult.error) throw new Error(projectsResult.error.message);
  const projectCodes = (projectsResult.data || []).flatMap(row => row.code ? [row.code] : []);
  const [statusResult, decisionsResult, recommendationsResult, profilesResult, failuresResult, benchmarkResult] = await Promise.all([
    db.from('act_funding_opportunity_current_status').select('feed_status'),
    db.from('act_grant_recommendation_decisions').select('decision, decision_scope, decided_at, opportunity_id, project_code').in('project_code', projectCodes),
    db.from('act_grant_recommendations_current').select('opportunity_id, project_code, computed_at').in('project_code', projectCodes),
    db.from('project_funding_profiles').select('completeness_status').eq('org_profile_id', org.id).eq('is_current', true),
    db.from('agent_runs').select('agent_id, status, started_at').gte('started_at', weekIso).order('started_at', { ascending: false }),
    db.from('act_opportunity_benchmark_cases').select('project_code, expected_label, review_status, label_source, reviewed_at').eq('benchmark_version', 'act-opportunity-v1').in('project_code', projectCodes),
  ]);
  for (const result of [statusResult, decisionsResult, recommendationsResult, profilesResult, failuresResult, benchmarkResult]) if (result.error) throw new Error(result.error.message);
  const statuses = statusResult.data || []; const decisions = decisionsResult.data || []; const profiles = profilesResult.data || [];
  const operationalDecisions = decisions.filter(row => row.decision_scope === 'operational');
  const historicalWins = decisions.filter(row => row.decision_scope === 'historical_evidence' && row.decision === 'won').length;
  const weeklyDecisions = operationalDecisions.filter(row => row.decided_at && row.decided_at >= weekIso);
  const stateCounts = operationalDecisions.reduce<Record<string, number>>((acc, row) => { acc[row.decision] = (acc[row.decision] || 0) + 1; return acc; }, {});
