import { createOpportunity, findGrantPipeline, getPipelines, updateOpportunity } from '@/lib/ghl';
import { getServiceSupabase } from '@/lib/supabase';

export interface PursueFundingInput { projectCode: string; opportunityId: string; amountSought: number; applicantEntity: string; relationshipOwner: string; nextAction: string; nextActionDue: string; grantscopeDecisionUrl: string; notionBriefUrl?: string | null; userId: string }
...
export async function pursueFundingInGhl(input: PursueFundingInput) {
  ...
  const contactId = process.env.GHL_GRANTS_TRIAGE_CONTACT_ID; if (!contactId && !existingResult.data?.ghl_opportunity_id) throw new Error('GHL_GRANTS_TRIAGE_CONTACT_ID is not configured');
  ...
  try {
    ...
    if (ghlOpportunityId) { await updateOpportunity(ghlOpportunityId, { pipelineStageId: stage.id, status: 'open', monetaryValue: input.amountSought }); operation = 'updated'; }
    ...
    return { handoffId: handoff.data.id, ghlOpportunityId, operation };
