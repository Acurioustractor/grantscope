...
export interface PursueFundingInput { projectCode: string; opportunityId: string; amountSought: number; applicantRouteId: string; relationshipOwner: string; funderContactEmail: string; nextAction: string; nextActionDue: string; grantscopeDecisionUrl: string; notionBriefUrl?: string | null; userId: string }
...
export function validatePursueFundingInput(input: Omit<PursueFundingInput, 'userId'>): string | null {
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(input.applicantRouteId)) return 'applicantRouteId must be a canonical route id'; if (!input.relationshipOwner.trim()) return 'relationshipOwner is required';
...
export async function pursueFundingInGhl(input: PursueFundingInput) {
  ...
  const [{ data: project, error: projectError }, { data: opportunity, error: opportunityError }, { data: opportunityRequirements, error: opportunityRequirementsError }, applicantRegistry] = await Promise.all([
  ...
  try {
    ...
    const decision = await db.from('act_grant_recommendation_decisions').upsert({ project_code: project.code, opportunity_id: input.opportunityId, decision: 'pursuing', decision_scope: 'operational', decision_origin: 'grantscope_pursue', decided_by: input.userId, decided_at: new Date().toISOString(), notes: `${input.nextAction} — due ${input.nextActionDue}; applicant ${applicantRoute.entity.name}; route ${applicantRoute.id}`, updated_at: new Date().toISOString() }, { onConflict: 'project_code,opportunity_id' }).select('id').single(); if (decision.error) throw new Error(decision.error.message);
    ...
    try {
      ...
      return { handoffId: handoff.data.id, ghlOpportunityId, operation, applicantRouteId: applicantRoute.id, applicantEntity: applicantRoute.entity.name, notionBrief };
    } catch (notionError) {
      return {
