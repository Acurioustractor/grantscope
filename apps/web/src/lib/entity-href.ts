 ...
 * power reports led a visitor to a sign-in page. /entities/<gs_id> is public (the one profile page).
 *
 * Order: the row's gs_id; else its ABN as AU-ABN-<abn> (the profile page resolves that by ABN
...
export function entityHref(ref: {
  ...
  if (ref.gsId) return `/entities/${encodeURIComponent(ref.gsId)}`;
  const abn = ref.abn?.replace(/\s/g, '');
  if (abn && /^\d{11}$/.test(abn) && !/^0+$/.test(abn)) return `/entities/AU-ABN-${abn}`;
