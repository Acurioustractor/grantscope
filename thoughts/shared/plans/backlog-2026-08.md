...
| **Lever 2 — the root-layout restructure** | The spike works: 78 routes flip to static, 519/5/11 becomes 441/83/11, one `(marketing)` group and 47 `git mv`s. But it optimises a cost nobody measures — **Web Analytics is not enabled**, so there is no route-level traffic data at all, and the only countable invocations are 248/month of cron. Meanwhile `auth.users` holds 25 and **exactly 1 has signed in in 30 days**, so the price (a signed-out flash while the nav hydrates client-side) is paid by one person. Doing the expensive thing to fix an unmeasured cost is the same shape as the four-month debris. | **The trigger is currently unpullable — say so rather than pretend.** Analytics shows real traffic to the 78 routes, **or** something else forces `app/layout.tsx` open. But wiring analytics (#289) was **closed 2026-08-19**: its Vercel build hangs deterministically at `Creating an optimized production build`, twice, with no error, while the same commit builds clean locally under Vercel's own memory ceiling with no cache. Prime suspect is the restored build cache, which predates the new dependency the root layout now imports; the untested fix is a redeploy with the cache unchecked. **Until someone wires analytics, the first half of this condition cannot be met** — which would quietly turn a conditional cut into a permanent one. Branch `feat/vercel-analytics` holds the working change. The spike branch is deleted; the method is in [the ticket](https://github.com/Acurioustractor/grantscope/issues/281) — root layout holds every route dynamic, and `nav.tsx` is already `'use client'` with `initialUserEmail` as a bare `useState` seed. |
