...
- [x] **Skills refresh + cull** (#139–#141): polish carries the two-family rule + Ben's rage-triggers; make-the-ask carries the Guidelines-PDF / ACNC-API / Gmail-sweep lessons; ports 3003→3013 in graph-studio/preflight/polish. FIVE ARCHIVED to _archive/2026-08-06-skills (commit, refresh-views, superdesign, graph-studio, leverage — RESTORE.md inside; SKILL.md renamed .archived so the scanner skips them). Seven survive: preflight, close, make-the-ask, polish, wedge, lighthouse, health. NOTE: commit + superdesign still appear from Ben's GLOBAL ~/.claude skills — a cross-project review for another session.
- Ben's UX taste saved to auto-memory: ben-ux-taste-one-desk (desk-centric, rail-owns-filters, plain words, kill label stacks)
