...
[->] **The `justice_*` rename: step 1 DONE end to end, awaiting push.** Migration `20260816090000` (dual-name, `recorded_grants_*` aliases on both matviews, nine-object rebuild) **APPLIED by Ben** — parity verified, 0 mismatches over 185,265 + 98,551 rows, totals $32.20bn/$31.59bn. App migrated (`a77c605`): 23 files onto new names; `mv_entity_total_funding` has NO app readers (refresh script only). NOT renamed: deserts/person-network/crosswalk/interlocks justice_* columns — different objects. **Name is `recorded_grants_*`** (state_grants would lie — foundation+NIAA rows; grants_* taken by GrantConnect). **Step 2 (drop justice_* old names + patch dependent viewdefs to read new names) is NOT WRITTEN YET — write it from live viewdefs only after the Vercel deploy of a77c605 is verified live, then Ben applies.**
