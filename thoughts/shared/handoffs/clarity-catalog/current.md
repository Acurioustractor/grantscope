...
- [x] **Slice 5 BUILT 2026-08-16 (branch `clarity-slice-5-row-viewer`) — row viewer + consent refusal, atomic as designed.** RPC `clarity_rows(key, limit)` (migration `migrations/2026-08-16-clarity-rows-rpc.sql`, APPLIED): SECURITY DEFINER, service_role-only (anon/authenticated verified false via `has_function_privilege`), catalogue-as-allowlist, kind-checked, ≤50 rows, vector columns excluded. The refusal lives IN the RPC: storytelling_consent domain + the 4-object list (SQL copy is authoritative for rows; visibility-floor.ts renders card copy — drift resolves toward consent, add SQL-side first). Withheld objects return a **per-flag consent census** instead of rows — reads the five transcripts flags per row without shipping a row; live numbers: ai analysis 1 yes/207 no, story creation 11 yes/197 no, unrecorded 0. UI `/clarity/o/[key]/rows` renders whatever the RPC decides (rows / refusal+census / empty-is-not-refusal / fault-is-not-refusal); malformed envelope renders as failure never empty-but-allowed (`row-viewer.ts` + tests). Object page Shape panel links "browse →" / "why withheld →". Smoked on 3013: transcripts refusal+census, foundations sample, unknown 404. No transcript content in any RSC payload — never fetched, not blurred (grant-award-history lesson).
- [x] **Slice 6b MERGED (#227, `7d2252a`) — code scanner.** `scripts/scan-clarity-code-refs.mjs`: 8,818 ref rows, refs_app/script/migration measured on all 1,479 (465 measured-unused). Trap: `clarity_code_ref.repo` CHECK wants 'civicgraph' not 'grantscope'. Re-run before trusting an orphan verdict (`scanned_at` dates it).
- [x] **Slice 7 MERGED (#228, `e5ab855`) — the findings stream.** `clarity_finding` table (migration `migrations/2026-08-16-clarity-finding.sql`, APPLIED) + `clarity_run_detectors()` (SECURITY DEFINER, service_role-only) + `/clarity/findings` + verdict API (`/api/clarity/findings`, requireAdminApi). First run: **235 undiscovered-join + 70 orphan findings**. Join detector grain is per (object, column) NOT per pair — `abn` in 77 objects would flood ~3K pair findings. Age-out: 30d default on proposed_at (plan left N open; constant `AGE_OUT_DAYS` in findings/types.ts), read-side only, verdicts never expire. Upsert key is plain columns (PostgREST onConflict can't hit expression indexes). Later detectors (weak seam, contradiction, dead end, community-led opportunity) not built.
- [x] **Slice 4 MERGED (#229, `2987857`) — the noun column.** `noun`/`noun_source`/`noun_proposed` on clarity_object (migration `migrations/2026-08-16-clarity-noun.sql`, APPLIED): 732 filed by the domain rule (mirror of nouns.ts DOMAIN_TO_NOUN — edit both together), **747 unfiled, 282 with name-heuristic proposals** (machine patterns before money, so agent_funding_queue proposes machine). `/clarity/unfiled` adjudication surface + `/api/clarity/nouns`; confirmations (and un-filings) recorded through the verdict columns — first use in anger, per plan. The index reads the COLUMN now, not nounFor() at render time. A proposal never files.
- [x] **Slice 2 MERGED (#230, `b156a10`) — inline edit, and the two shipped bugs its HTTP verification caught.** EditableCurated (purpose/caveat/grain/join_keys) + PATCH /api/clarity/object with curated_by/at provenance (migration APPLIED). Column-scoped GRANT UPDATE = DB enforces documentation-never-overwrites-measurement. **Bugs found by real HTTP (the plan's demand paid twice): service_role was SELECT-only on clarity_object (slice 4's nouns route 500'd on every write, never having served a request) and clarity_object.verdict is enum {keep,suspect,cruft} — 'confirmed' never existed; filing now records 'keep' with the noun action in verdict_reason.** requireAdminApi honors the local-dev bypass (no-session local only, never Vercel/prod). First real filing on record: agent_runs → machine. Vercel flake note: Google Fonts fetch failure during build = retrigger with empty commit, not a code problem.
- **PART-1 PLAN COMPLETE: slices 0–7 (incl. 6b) all merged.**
- [x] **Slice 8 BUILT 2026-08-16 (branch `clarity-slice-8-owner-app`) — owner_app measured, proposed, confirmable.** Scanner now REPO-AWARE (civicgraph + ~/Code/JusticeHub; missing JH checkout warns "one-eyed"). Trap: JH's generated `database.types.ts` matched 1,234 names — the schema reflected back, not usage; `!**/database.types.ts` + `!**/*.generated.*` excluded. **Two-eyed scan rescued 220 false orphans (465→245 zero-ref); 42 open orphan findings machine-withdrawn (human verdicts never touched); detector re-run = 28 orphans, exactly 70−42.** Proposals from app+script refs only (migrations = custody of schema, not ownership): 437 civicgraph / 244 justicehub / 158 both / 640 no-evidence-stays-neither. `/clarity/owners` + `/api/clarity/owners` (set + confirm_all; batch never bulldozes a single override). owner_app/owner_* added to the column-scoped grant. HTTP-verified `set` (state_tenders→justicehub); `confirm_all` deliberately NOT fired — 437 mass filings are Ben's adjudication. Ben: hit the three Confirm-all buttons on /clarity/owners if the groups look right.
- Adjudication backlog for Ben: 747 unfiled at /clarity/unfiled (282 with proposals), 263 findings at /clarity/findings (235 joins + 28 orphans post-withdrawal), 839 owner proposals at /clarity/owners.
- [x] **Slice 9 BUILT 2026-08-17 (branch `clarity-slice-9-project-codes`) — project codes, cross-repo.** Wiki declares (`act-global-infrastructure/config/project-evidence.json`, committed `ad91b14` there, NOT pushed); `scripts/sync-project-evidence.mjs` mirrors both files into `clarity_project_code` + `clarity_object.project_codes`, validating BOTH ends and refusing (not skipping) unknown codes/objects. Seeded from documented facts only: ACT-JH, ACT-GD, ACT-GS, ACT-EL → 16 objects. **The killer metric is live: 70 of 74 codes have ZERO evidence** at /clarity/projects (read-only on purpose — the claim belongs to the work, not a button). Object page provenance shows project chips. To declare more: edit the wiki file, re-run the sync.
- [x] **Slice 10 BUILT 2026-08-17 (branch `clarity-slice-10-stories`) — story ↔ project, day-shift with Ben's three rulings:** both corpora from day one (`story_table` discriminator: stories | transcripts), the declaration lives in `clarity_story_project_link` in the DB (story ids in a public git repo would themselves disclose — wiki-file pattern rejected), surfacing is published-only titles (anything else is a COUNT, never a title). API `/api/clarity/stories` link/unlink validates both ends and never reads story content (selects id only). `/clarity/stories` linking surface lists ONLY published stories; transcripts link by id from systems already holding the right to see them. `/clarity/projects` gained the story strip: what we say next to what the data says, project-mediated, nobody triangulated. No FK to clarity_project_code (sync rebuilds it DELETE+INSERT); API validates instead. HTTP-verified: link, strip render, unknown-code refusal, unlink.
- [x] **Adjudication queues cleared 2026-08-17, Ben-delegated:** owners — all 839 proposals confirmed (437 cg / 245 jh incl. state_tenders / 157 both; 647 stay neither). Unfiled — 279 of 282 proposals filed after review with 66 corrections (maintenance-verb functions → machine; `v_clarity_board*` are the question board not people; `se_directory`/`community_directory_orgs` → organisations; **the `history`-contains-`story` trap re-hit**: v_act_*_history were proposed evidence → money; TRAP: function object_keys carry FULL SIGNATURES — bare-name IN-lists silently miss them, corrected via split_part). 3 skipped as ambiguous; 475 remain unfiled with no proposal (per-item judgement, another day). Findings — **empathy-ledger was a scanner blind spot; third repo added (repo CHECK widened), rescan changed nothing (all 28 orphans survive a three-app scan) → 28 orphans CONFIRMED; joins: 116 domain-entity-key gaps CONFIRMED (50 are project_code — the slice-9 lane), 119 external-id/auth/polymorphic DISMISSED as no-edge-by-design.** All verdicts stamped 'claude (Ben-delegated bulk adjudication)'.
- [x] **Slice 10 MERGED (#234, `a2c9a94`). THE PLAN IS COMPLETE — slices 0–10 all on main.**
- Wiki repo: all pushed through `df1d08a`.
...
- ~~The 35 unfiltered money views~~ **AUDITED 2026-08-16** — `thoughts/shared/data-map/unfiltered-money-views-audit.md`. 7 SEVERE (worst: mv_revolving_door, 19 consumers, TOTAL RPA $4.74bn in a ranking; mv_yj_report_* feeds the PUBLIC snapshot with $11.4bn dept budget rows as recipients; mv_funding_outcomes_summary shows $258.5bn on one entity). Fix order in the doc; each is a Ben-applies rebuild. **Fix #1 APPLIED 2026-08-16: mv_revolving_door** — 6,988→3,589 rows (49% artifact), donated $21.08bn→$0.75bn (28x, receipt_type), funded $50.54bn→$22.85bn; TOTAL RPA gone, QR remains by design (genuine grant rows); top of ranking now KPMG/EY/Telstra. v_goods_relationship_power recreated with grants, 104 rows. **Fix #2 APPLIED 2026-08-16: the mv_yj_report_* family** (migration 20260816120000) — recipients $43.85bn→$915.7M (48x; matches registered youth-justice-total within $128k), remoteness $18.32bn→$825M, ACCO share 12.0→11.5% (claim survives), state trio barely moved (hand rules had held). coverage untouched by design (a manifest). Snapshot REBUILT (data/report-snapshots/youth-justice.json, −2,291 lines of dept/total rows; revolvingDoor section refreshed too). **Fix #3 APPLIED: mv_funding_outcomes_summary rewritten** (20260816130000) — $377.00bn→$32.20bn, EXACTLY equal to epi.recorded_grants_dollars (independent derivations agree to the dollar); proven=1 invariant held; fan-out fixed via per-lane CTEs. **Fixes #4-6 APPLIED (20260816140000):** proven_suppliers & grant_contract_overlap $54.20bn→$22.29bn each (agree within $2 — independent-derivation check), both 4,868→2,394 rows; youth_justice_entities $6.65bn→$3.15bn. proven_suppliers EXISTS presence check filtered too (a dept qualified as supplier through it). **RESOLVED 2026-08-17 (Ben: revoke): proven_suppliers anon+authenticated grants revoked (migration 2026-08-17-proven-suppliers-grant-posture.sql, applied); both consumers read via service key, SE page smoked 200.** ALL actionable SEVERE fixes from the money-views audit are now DONE. **Person trio FIXED 2026-08-16 (migration 20260816150000, applied):** three money filters on every justice read, receipt_type on every donation read, fan-out de-collide (DISTINCT person↔ABN pairs + is_nominee_block anti-join via person_identities — the flag is NOT on person_roles, CLAUDE.md's table listing is wrong there). Donations $0.94bn→$0.52bn (person) and $5.81bn→$0.24bn (crosswalk); justice spot-checks tie to the dollar. 10 downstream relations recreated verbatim incl. third-order mv_foundation_readiness. Ops lesson: the rebuild is ONE ~50min transaction (mv_trustee_grantee_chain alone ~40min); run via background psql, never a foreground shell (10-min cap kills the client and ORPHANS the server-side build — the backend keeps running with no client to send COMMIT and must be pg_terminate'd, Ben verb required). Still open, different lane: ranking top rows are name-collision people (identity `_v2` lane); slice 7 detectors can now also use the measured refs.
