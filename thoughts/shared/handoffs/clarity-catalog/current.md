...
[->] **Slice G is BUILT and committed (`c225cbd`), not yet pushed.** Themes section (12 theme pages, report + question counts, links to `/reports/theme/[slug]`) now leads `/clarity`, above the six-noun object index. Question counts come from `v_clarity_board_cards.subject` mapped through each theme's `questionSubjects`. Verified rendering on 3013 ("Youth Justice — 17 reports · 6 q"). Next: push, then H (ACT cross-project view in `/org/act`) or the `justice_*` column rename.
