...
[->] **The `justice_*` rename: step 1 DONE end to end, awaiting push.** Migration `20260816090000` (dual-name, `recorded_grants_*` aliases on both matviews, nine-object rebuild) **APPLIED by Ben** — parity verified, 0 mismatches over 185,265 + 98,551 rows, totals $32.20bn/$31.59bn. App migrated (`a77c605`): 23 files onto new names; `mv_entity_total_funding` has NO app readers (refresh script only). NOT renamed: deserts/person-network/crosswalk/interlocks justice_* columns — different objects. **Name is `recorded_grants_*`** (state_grants would lie — foundation+NIAA rows; grants_* taken by GrantConnect). **Step 2 is WRITTEN and committed (`20260816100000`, commits `cfeb8cf`+`80b7603`), NOT applied.** Drops the seven justice_* names; dependent views keep their OUTPUT columns (deserts.justice_dollars, interlocks.total_justice_dollars, v_goods_rel_power.in_justice_funding) — only internal epi reads move. **Two stored-SQL readers found and patched in-transaction: org-total-funding's answer_sql and get_power_holder()** (output keys preserved). **Regression caught & fixed (`e73ea9e`): goods-relationship-power.ts was flipped ahead of its view — reverted with comment; it deliberately stays on in_justice_funding.** Rename deploy 3c3b852 verified READY in production → apply gate OPEN. Ben applies 100000 same as 090000.
