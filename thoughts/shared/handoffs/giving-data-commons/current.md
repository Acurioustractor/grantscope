...
**Updated:** 2026-06-09T09:23:22+10:00
...
**Branch:** **main** (clean) — PRs #57–#64 all **MERGED** (this session: #63 OP6, #64 OP6 polish). HEAD `1872cdb`. Working tree clean except the 2 long-standing untracked data leftovers (`data/grant-eligibility-cache.jsonl`, `data/state-tenders/`). OP6 + polish branches pruned (local+remote). 6 migrations applied live this session (`20260609020000`–`050000`).
**Test:** cd apps/web && npx tsc --noEmit && npx vitest run

### Now
[->] **Leverage estate fully mined out — OP1–OP10 + OP6 all shipped.** Nothing in the leverage plan in flight; re-run `/leverage` only on genuinely new data (a new crawl/enrichment, not an MV refresh). **Three live threads carried forward:** (1) **Lighthouse/NIAA** — pack + demo tender-pack ready (`thoughts/shared/prospects/niaa/`); the next step is **Ben's outreach** (find IPP contact, render PDF, send — Tier 3, day-shift). (2) **Agent fleet** — psql/executor/priority all fixed this session; **only the stuck-`running` reaper remains** (scoped in health-backlog A5, code+restart, not built) + a one-time cleanup of orphaned `running` rows. (3) **C5 residual** = B3 (313K entities with no `lga_code`/postcode → address enrichment; confirmed *not* cheaply derivable — needs ABR/web, low yield; A1/A5 + B1 ABN backfill are higher leverage).

### This Session (2026-06-09, late) — OP6 shipped, C5 data-quality fix, agent-fleet triage
Marathon session. Shipped the last leverage candidate (OP6), then followed the thread it exposed into a real data-integrity fix (C5), then triaged the failing agent fleet (A1/A5). Ben merged each PR + authorized each prod write with explicit verbs; multiple Tier-2/3 writes were (correctly) gated by the auto-mode classifier until he said the word.
- [x] **OP6 BUILT + MERGED** (PR #63, merge `385f06d`) — **"Who's Already There"** section on `/reports/funding-deserts`: the named, profile-linked list of community-controlled orgs in the worst-100 desert LGAs (was a latent count). Sourced from `mv_entity_power_index` (same source the MV counts) so the named list **reconciles exactly** with `community_controlled_entities`. Shared SQL+mapper in `lib/funding-deserts.ts` (page + `/api/data/funding-deserts` `communityControlledInDeserts`). No new MV/migration. Commit `a1000ec`. **Polish** (PR #64, merge `2a64654`): title-cased entity-type labels (`Indigenous Corporation` etc.).
- [x] **Funding-deserts "Unknown" state bug → fixed (the thread OP6 exposed).** 227 of 584 desert LGAs showed state "Unknown", and **phantom zero-entity LGA splits** topped the worst-deserts list (a ghost "Barkly / blank / 0 entities / 190" outranking the real "Barkly / NT / 33"). Root cause: MV keyed LGAs on unreliable `postcode_geo.state` (null ~9%, sometimes wrong); the ABS `lga_code` first digit is the reliable key. Migration `20260609020000`: backfilled 1,071 null `postcode_geo.state` from `lga_code` + refreshed the 2 desert MVs → 227 Unknowns → **0**, phantoms collapsed. OP6 figures moved 102→565 (page computes live; copy softened "nearly all"→"the overwhelming majority"). In PR #64 (`b863e7e`).
- [x] **C5 — full state↔lga_code contradiction fix (health-backlog C5 DONE).** Beyond the nulls, ~130 `postcode_geo` + **6,949 `gs_entities`** rows had state contradicting `lga_code` (Laverton coded NT but lga_code 54970 = WA; Albury VIC→NSW; case-variants "Qld"→"QLD"). Fixed in lockstep across both tables (migrations `20260609030000` + `…040000`), **all 38 MVs refreshed** (0 failures). Laverton consolidated to 1 WA/101-entity row (NT phantom gone); OP6 settled at **665** (reconciles 665=665, 89% Indigenous, 374 on $0). Commits `caa6934`, `9b61b40`. **B3 (313K no-lga_code) confirmed NOT cheaply fixable** (only 64 postcode-derivable) → left as address-enrichment backlog.
- [x] **Lighthouse — NIAA demo tender-pack added** (`thoughts/shared/prospects/niaa/tender-pack-demo.md`, commit `1aa2c0e`): "First Nations research/evaluation/advisory" category + IPP citation + grounded certified-Indigenous supplier shortlist (incumbents + new defensible options). Headline re-verified (132 SE / 108 cert / 124 Indigenous / $72.9M / 364 contracts).
- [x] **Agent-fleet triage (A1/A5) — root-caused; 3 of 4 fixed.** The "3% success / silent failure" signal was **three** distinct causes, two environmental not code: (1) **`spawn psql ENOENT`** — pm2 orchestrator's minimal PATH omitted Homebrew → every psql-shelling agent died. Fixed: `PATH` env on the orchestrator in `ecosystem.config.js` (`d7a2aaa`), restarted + saved, **proven live** (queued Trust Remediation ran >220s past the old 8.6s→ENOENT death, 0 ENOENT in logs). (2) **PostgREST schema-cache error** blocked `claim_next_task` → executor claimed *nothing* (shaken loose by this session's DDL+MV refresh). Cleared with `NOTIFY pgrst, 'reload schema'`. (3) **`agent_tasks_priority_check`** — `watch-outcomes-changes` priority 50 (>10) spammed the scheduler. Fixed: clamped to 10 (`20260609050000`). Commit `1872cdb`. **Open: A5 stuck-`running` reaper** — orchestrator kills child agents on restart without terminalizing `agent_runs` → orphaned `running` rows (the real "silent" signal; enrich agents run clean standalone). Reaper scoped in backlog, not built.
...
> **Leverage estate fully mined: OP1–OP10 + OP6 all ✅.** Tree clean on `main` (`1872cdb`). **Re-run `/leverage` only after genuinely new data lands** (a crawl/enrichment, not an MV refresh). The live threads now are buyer-revenue (lighthouse) + agent-fleet finish-up, not more leverage mining.
1. **Lighthouse / NIAA — Ben's outreach (Tier 3, day-shift).** Pack + demo tender-pack are ready (`thoughts/shared/prospects/niaa/` — one-pager, email-draft, tender-pack-demo, PIPELINE). Next: find a named IPP/procurement contact, render to PDF, send. *Drafts only from a session — the human sends.* Second iron if wanted: Services Australia (highest non-Defence SE count).
2. **Agent A5 — build the stuck-`running` reaper** (`docs/health-backlog.md`). On orchestrator startup + a periodic tick, mark `agent_runs` that are `running` with stale `started_at` (no live child) → `failed`. Then one-time-clean the existing orphaned `running` rows and **re-measure true agent success rates** (the old "3%" was inflated by orphans + the now-fixed psql/executor/priority bugs). Then B1 (ABN backfill) / remaining A1 are the next data-quality leverage.
3. (Watch) **Confirm the agent fixes held** — check `/health` or `agent_runs` after the next scheduled cycle: Trust Remediation / Refresh MVs / Youth Justice chain should now go green (psql PATH), and `watch-outcomes-changes` should schedule (priority fixed). If the PostgREST schema-cache error recurs after heavy DDL/refresh, re-issue `NOTIFY pgrst, 'reload schema'` (consider auto-issuing it at the end of the nightly MV-refresh job).
4. (Housekeeping) Decide on the 2 long-standing untracked data leftovers (`data/grant-eligibility-cache.jsonl`, `data/state-tenders/`) — gitignore, commit, or bin.
