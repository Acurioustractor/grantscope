...
| ~~`mv_person_network` + `mv_person_entity_network` + `mv_person_entity_crosswalk`~~ | 3 + 4 + 1 | **FIXED 2026-08-16** (migration `20260816150000_person_trio_money_filters.sql`, applied) | All justice reads gained the three filters, all donation reads gained `receipt_type`, and mv_person_network's fan-out (duplicate person↔ABN role rows multiplying sums; nominee-block identities attributed thousands of companies' money) fixed via a DISTINCT pair set with a `person_identities.is_nominee_block` anti-join. Person donations $0.94bn→$0.52bn; crosswalk donations $5.81bn→$0.24bn (24x). Crosswalk justice spot-checks equal direct filtered sums to the dollar. 10 downstream relations recreated verbatim (incl. third-order `mv_foundation_readiness`, which pg_depend on the trio alone does not show). REMAINING, out of scope: the ranking's top rows are name-collision people (DAVID SMITH, 68 boards) — identity resolution, the `_v2` lane's job, not a money defect. |
