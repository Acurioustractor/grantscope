...
> Last synced: 2026-09-05. **Do not edit this section directly.** Edit the upstream file and run `node scripts/sync-act-context.mjs --apply`. Downstream edits get overwritten.
...
### Shared Supabase project (decided 2026-09-05, platform review)
One project, `tednluwflfhxyucgwigh`, is the database for CivicGraph, JusticeHub, Harvest, studio, act-farm and the ACT
business systems, and Empathy Ledger reads into it. Six rules hold across every repo that touches it:
- **One migrations home:** `grantscope/supabase/migrations/<14-digit version>_<name>.sql`. A change made from any other repo
  or via the MCP `apply_migration` tool is committed there with the same version in the same session, or it is an orphan.
- **One apply path:** `grantscope/scripts/db-apply.sh <file>` (psql, then the tracker). Tier 3: Ben's verb. `/db-apply` in grantscope.
- **Parity is checked** by `grantscope/scripts/check-migration-parity.mjs`, in `/preflight` and in grantscope CI.
- **Nothing private is ever granted to anon:** `xero_*`, `ghl_*`, `linkedin_*`, `communications_*`, `email_*`, receipts,
  `project_funding_*`, `project_pipelines`, `goods_relationships`, `funder_context_snapshot`, or any view over them.
  New views are `WITH (security_invoker = true)`. Exposure is measured by RLS state, never by grant.
- **Keys:** legacy JWT keys are disabled on the project; use `sb_publishable_` / `sb_secret_` keys. Trailing newlines in env values break strict comparisons.
- **The register is live** at civicgraph.app/ops/schema: owner, consumers, size and what the public key can read, per object.
  Findings and the phased plan: `grantscope/thoughts/shared/findings/supabase-platform-review-2026-09-05.md`.
