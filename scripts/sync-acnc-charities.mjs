...
/**
 * Sync ACNC Register → Supabase acnc_charities table
 *
 * Downloads the FULL ACNC charity register CSV (~63,500 records)
 * and upserts ALL charities (not just foundations) to acnc_charities.
 *
 * Usage: node scripts/sync-acnc-charities.mjs [--dry-run]
 */

import 'dotenv/config';
import { createClient } from '@supabase/supabase-js';
import { downloadACNCRegister } from '../packages/grant-engine/src/foundations/acnc-importer.ts';
...
function log(msg) {
  console.log(`[acnc-charities] ${msg}`);
}

// Foundation detection (reused from acnc-importer.ts)
...
// Purpose mapping: CSV column → { db_column, label }
const PURPOSE_MAP = [
...
// Beneficiary mapping: CSV column → { db_column, label }
...
function parseDate(s) {
  if (!s || s === '') return null;
  // Try DD/MM/YYYY format first (ACNC uses this)
...
async function main() {
  log('Starting ACNC full register sync...');
  ...
  // Step 3: Map and dedup by ABN (CSV has multiple rows per org for different address types)
  const byAbn = new Map();
  ...
  if (DRY_RUN) {
    ...
    for (const r of unique.slice(0, 10)) {
      log(`  ${r.abn} | ${r.name} | ${r.charity_size} | purposes: ${r.purposes.join(', ')}`);
    }
    return;
  }
