 ...
 * identities (clustering core in scripts/lib/person-cluster.mjs), then reports
...
import { THRESHOLDS, makeQuery, fetchName, cluster } from './lib/person-cluster.mjs';

const supabase = createClient(
  process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL,
  process.env.SUPABASE_SERVICE_ROLE_KEY
);
const { qPaged } = makeQuery(supabase);
...
function report(res) {
  ...
  console.log(`  block officers (>=${THRESHOLDS.HIGH_DEGREE} shared boards): ${blockOfficers.map(o => `${o.name}(${o.degree})`).join(', ') || 'none'}`);
