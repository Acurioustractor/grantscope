-- Recreate 8 MVs that were dropped by CASCADE when rebuilding mv_entity_power_index
-- Run in dependency order

-- ═══ 1. mv_revolving_door (no deps on dropped MVs) ═══
...
lobbyists AS (
  ...
  FROM gs_relationships r WHERE r.relationship_type = 'lobbies_for'
),
donors AS (
  SELECT ge.id as entity_id, COUNT(*) as donation_count, SUM(pd.amount) as total_donated,
    array_agg(DISTINCT pd.donation_to ORDER BY pd.donation_to) as parties
  FROM political_donations pd JOIN gs_entities ge ON ge.abn = pd.donor_abn
  WHERE pd.donor_abn IS NOT NULL GROUP BY ge.id
),
contractors AS (
  SELECT ge.id as entity_id, COUNT(*) as contract_count, SUM(ac.contract_value) as total_contracts,
    COUNT(DISTINCT ac.buyer_name) as distinct_buyers
  FROM austender_contracts ac JOIN gs_entities ge ON ge.abn = ac.supplier_abn
  WHERE ac.supplier_abn IS NOT NULL GROUP BY ge.id
),
funded AS (
  SELECT gs_entity_id as entity_id, COUNT(*) as funding_count, SUM(amount_dollars) as total_funded
  FROM justice_funding WHERE gs_entity_id IS NOT NULL GROUP BY gs_entity_id
)
SELECT ge.id, ge.gs_id, ge.canonical_name, ge.entity_type, ge.abn, ge.state, ge.lga_name, ge.is_community_controlled,
  (l.entity_id IS NOT NULL) as lobbies, (d.entity_id IS NOT NULL) as donates,
  (c.entity_id IS NOT NULL) as contracts, (f.entity_id IS NOT NULL) as receives_funding,
  (l.entity_id IS NOT NULL)::int + (d.entity_id IS NOT NULL)::int + (c.entity_id IS NOT NULL)::int + (f.entity_id IS NOT NULL)::int AS influence_vectors,
  COALESCE(d.total_donated, 0) as total_donated, COALESCE(d.donation_count, 0) as donation_count, d.parties as parties_funded,
  COALESCE(c.total_contracts, 0) as total_contracts, COALESCE(c.contract_count, 0) as contract_count, COALESCE(c.distinct_buyers, 0) as distinct_buyers,
  COALESCE(f.total_funded, 0) as total_funded, COALESCE(f.funding_count, 0) as funding_count,
  ((l.entity_id IS NOT NULL)::int * 5 + (d.entity_id IS NOT NULL)::int * 3 + (c.entity_id IS NOT NULL)::int * 2 + (f.entity_id IS NOT NULL)::int * 1 +
    CASE WHEN COALESCE(d.total_donated, 0) > 100000 THEN 3 WHEN COALESCE(d.total_donated, 0) > 10000 THEN 1 ELSE 0 END +
    CASE WHEN COALESCE(c.total_contracts, 0) > 10000000 THEN 3 WHEN COALESCE(c.total_contracts, 0) > 1000000 THEN 1 ELSE 0 END +
    LEAST(array_length(d.parties, 1), 5)
  ) AS revolving_door_score
FROM gs_entities ge
LEFT JOIN lobbyists l ON l.entity_id = ge.id LEFT JOIN donors d ON d.entity_id = ge.id
LEFT JOIN contractors c ON c.entity_id = ge.id LEFT JOIN funded f ON f.entity_id = ge.id
WHERE (l.entity_id IS NOT NULL)::int + (d.entity_id IS NOT NULL)::int + (c.entity_id IS NOT NULL)::int + (f.entity_id IS NOT NULL)::int >= 2;

CREATE INDEX IF NOT EXISTS idx_revolving_door_score ON mv_revolving_door (revolving_door_score DESC);
CREATE INDEX IF NOT EXISTS idx_revolving_door_vectors ON mv_revolving_door (influence_vectors DESC);
CREATE INDEX IF NOT EXISTS idx_revolving_door_entity_type ON mv_revolving_door (entity_type);

-- ═══ 2. mv_person_entity_network (no deps) ═══
...
person_entity_count AS (
  SELECT person_name_normalised, COUNT(DISTINCT entity_id) as board_count FROM person_boards GROUP BY person_name_normalised
),
entity_financial AS (
  SELECT e.id as entity_id,
    COALESCE(ac.contract_total, 0) as procurement_dollars, COALESCE(ac.contract_count, 0) as contract_count,
    COALESCE(jf.justice_total, 0) as justice_dollars, COALESCE(jf.justice_count, 0) as justice_count,
    COALESCE(pd.donation_total, 0) as donation_dollars, COALESCE(pd.donation_count, 0) as donation_count
  FROM gs_entities e
  LEFT JOIN LATERAL (SELECT SUM(contract_value) as contract_total, COUNT(*) as contract_count FROM austender_contracts WHERE supplier_abn = e.abn AND e.abn IS NOT NULL) ac ON true
  LEFT JOIN LATERAL (SELECT SUM(amount_dollars) as justice_total, COUNT(*) as justice_count FROM justice_funding WHERE gs_entity_id = e.id) jf ON true
  LEFT JOIN LATERAL (SELECT SUM(amount) as donation_total, COUNT(*) as donation_count FROM political_donations WHERE donor_abn = e.abn AND e.abn IS NOT NULL) pd ON true
...
SELECT pb.person_name_normalised, pb.person_name_display, pb.entity_id, pb.entity_name, pb.entity_abn,
  ...
  ef.procurement_dollars, ef.contract_count, ef.justice_dollars, ef.justice_count, ef.donation_dollars, ef.donation_count,
...
-- ═══ 3. mv_person_influence (deps: mv_person_entity_network) ═══
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_person_influence AS
SELECT person_name_normalised, MIN(person_name_display) as person_name,
  COUNT(DISTINCT entity_id) as board_count,
  COUNT(DISTINCT entity_id) FILTER (WHERE is_community_controlled) as acco_boards,
  ARRAY_AGG(DISTINCT entity_type) as entity_types, ARRAY_AGG(DISTINCT source) as data_sources,
  SUM(procurement_dollars) as total_procurement, SUM(contract_count) as total_contracts,
  SUM(justice_dollars) as total_justice, SUM(donation_dollars) as total_donations,
  MAX(influence_score) as max_influence_score,
  (SUM(procurement_dollars) > 0)::int + (SUM(justice_dollars) > 0)::int + (SUM(donation_dollars) > 0)::int as financial_system_count
FROM mv_person_entity_network GROUP BY person_name_normalised ORDER BY max_influence_score DESC;

CREATE UNIQUE INDEX IF NOT EXISTS idx_pi_person ON mv_person_influence (person_name_normalised);
CREATE INDEX IF NOT EXISTS idx_pi_influence ON mv_person_influence (max_influence_score DESC);
CREATE INDEX IF NOT EXISTS idx_pi_board_count ON mv_person_influence (board_count DESC);
CREATE INDEX IF NOT EXISTS idx_pi_financial_systems ON mv_person_influence (financial_system_count DESC);
...
WITH ndis_by_lga AS (
  SELECT lga_name, state, SUM(participant_count) AS ndis_participants, COUNT(*) AS lga_entries
  ...
  SELECT state_code AS state, COUNT(DISTINCT abn) AS registered_providers, COUNT(DISTINCT provider_detail_id) AS registration_count
  FROM ndis_registered_providers GROUP BY state_code
  ...
  SELECT COALESCE(ge.lga_name, 'Unknown') AS lga_name, COALESCE(ge.state, 'Unknown') AS state,
    COUNT(*) AS disability_interventions, COUNT(*) FILTER (WHERE ai.type = 'Wraparound Support') AS wraparound_interventions,
    COUNT(*) FILTER (WHERE ai.type = 'Community-Led') AS community_led_interventions, AVG(ai.portfolio_score) AS avg_evidence_score
...
CREATE INDEX IF NOT EXISTS idx_disability_landscape_lga ON mv_disability_landscape(lga_name);

-- ═══ 6. mv_foundation_need_alignment (deps: mv_foundation_grantees, mv_funding_deserts) ═══
...
-- ═══ 7. mv_foundation_scores (deps: mv_foundation_grantees, mv_foundation_need_alignment, mv_evidence_backed_funding, mv_trustee_grantee_chain) ═══
...
-- ═══ 8. mv_foundation_readiness (deps: mv_foundation_grantees, mv_foundation_scores) ═══
...
CREATE INDEX IF NOT EXISTS idx_fr_type ON mv_foundation_readiness (type);
