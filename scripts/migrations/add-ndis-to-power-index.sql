-- add-ndis-to-power-index.sql
-- Add NDIS as the 8th system in the cross-system power concentration index.
--
-- NDIS providers are matched to gs_entities via ABN from ndis_registered_providers.
-- This extends mv_entity_power_index with:
--   - in_ndis_provider flag
--   - ndis_provider_count (how many NDIS registrations the entity holds)
--   - NDIS included in system_count calculation
--   - NDIS weighted at 2 (strong signal) in power_score
--
-- Usage:
...
--     -f scripts/migrations/add-ndis-to-power-index.sql
...
-- 8. NDIS registered providers (NEW)
ndis AS (
  SELECT ge.id as entity_id,
  ...
  JOIN gs_entities ge ON ge.abn = nrp.abn
  WHERE nrp.abn IS NOT NULL
    AND nrp.registration_status = 'Approved'
  GROUP BY ge.id
),

-- Board/directorship connections
boards AS (
...
SELECT
  ...
  -- Power score: number of systems present in (now 8 systems)
  ...
  (t.entity_id IS NOT NULL)::int +
  (n.entity_id IS NOT NULL)::int AS system_count,
  ...
  COALESCE(n.ndis_provider_count, 0) as ndis_provider_count,
  ...
  (
    ...
    (n.entity_id IS NOT NULL)::int * 2 +          -- NDIS = strong signal ($35B/yr system)
...
CREATE UNIQUE INDEX idx_power_index_id ON mv_entity_power_index (id);
