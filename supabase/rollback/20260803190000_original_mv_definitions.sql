-- Original materialized-view definitions captured 2026-08-03, immediately before
-- migration 20260803190000_fix_foundation_grantee_attribution.sql.
--
-- Kept so the change is reversible. NOTE: mv_foundation_grantees below still
-- contains the phantom justice_funding branch that migration removed. Restoring
-- this file reinstates the bug. It exists to recover the six dependent views if
-- a rebuild goes wrong, not to be replayed wholesale.

-- ===== mv_foundation_grantees =====
CREATE MATERIALIZED VIEW public.mv_foundation_grantees AS  SELECT f.id AS foundation_id,
...
UNION ALL
 SELECT DISTINCT f.id AS foundation_id,
    f.name AS foundation_name,
    f.acnc_abn AS foundation_abn,
    f.total_giving_annual,
    ge_recip.id AS grantee_entity_id,
    ge_recip.gs_id AS grantee_gs_id,
    ge_recip.canonical_name AS grantee_name,
    ge_recip.abn AS grantee_abn,
    ge_recip.entity_type AS grantee_type,
    ge_recip.state AS grantee_state,
    ge_recip.is_community_controlled AS grantee_community_controlled,
    jf.amount_dollars::bigint AS grant_amount,
    jf.financial_year AS grant_year,
    'justice_funding'::text AS source_dataset,
    'justice_funding'::text AS link_method
   FROM foundations f
     JOIN justice_funding jf ON upper(jf.program_name) ~~ (('%'::text || upper(split_part(f.name, ' '::text, 1))) || '%'::text) AND upper(jf.program_name) ~~ '%FOUNDATION%'::text
     JOIN gs_entities ge_recip ON ge_recip.id = jf.gs_entity_id
  WHERE f.acnc_abn IS NOT NULL AND jf.gs_entity_id IS NOT NULL AND jf.amount_dollars > 0::numeric AND length(split_part(f.name, ' '::text, 1)) > 3;
CREATE INDEX idx_fdn_grantee_foundation ON public.mv_foundation_grantees USING btree (foundation_id);
CREATE INDEX idx_fdn_grantee_grantee ON public.mv_foundation_grantees USING btree (grantee_entity_id);
CREATE INDEX idx_fdn_grantee_abn ON public.mv_foundation_grantees USING btree (foundation_abn);
CREATE INDEX idx_fdn_grantee_amount ON public.mv_foundation_grantees USING btree (grant_amount DESC NULLS LAST);
CREATE INDEX idx_fdn_grantee_community ON public.mv_foundation_grantees USING btree (grantee_community_controlled) WHERE (grantee_community_controlled = true);
