-- Dual-name transition, step 1 of 2: recorded_grants_* alias columns added ALONGSIDE the
-- justice_* columns on mv_entity_total_funding and mv_entity_power_index.
--
-- Apply:
--   source .env && PGPASSWORD="$DATABASE_PASSWORD" psql -h aws-0-ap-southeast-2.pooler.supabase.com \
--     -p 5432 -U "postgres.tednluwflfhxyucgwigh" -d postgres \
--     -f supabase/migrations/20260816090000_dual_name_recorded_grants_columns.sql
--
-- WHY DUAL-NAME. The columns hold money from the justice_funding TABLE — 81% a whole-of-QLD
-- grants register, 19.7% topic-tagged. "justice" in the name is the same lie the app labels told
-- until 2026-08-16 ("Recorded Grants" now). A flag-day column rename would break every deployed
-- PostgREST caller between migration and deploy, so: add recorded_grants_* aliases now, migrate
-- the app, then drop the justice_* names in step 2 (20260816100000, DO NOT APPLY until the app
-- deploy that uses the new names is verified live).
--
-- The name is recorded_grants_*, not state_grants_* (the register includes foundation and NIAA
-- federal rows) and not grants_* (taken: grants_total/grants_count already mean GrantConnect).
--
-- Derived programmatically from 20260816060000 and 20260816080000 — each SELECT is reproduced
-- verbatim and wrapped, aliases added in the wrapper. Indexes and GRANTs reproduced per object;
-- live relacl re-checked 2026-08-16: mv_entity_total_funding carries agent_readonly only,
-- mv_entity_power_index carries service_role + agent_readonly. Reproduced exactly — this
-- migration changes names, not privileges.
...
SELECT sub.*,
       sub.justice_total AS recorded_grants_total,
       sub.justice_count AS recorded_grants_count
FROM (
 WITH grants AS (
...
COMMENT ON MATERIALIZED VIEW public.mv_entity_total_funding IS
  'Per-entity funding rollup. recorded_grants_total (alias of the deprecated justice_total) is '
  'money from the justice_funding TABLE — 81% the whole-of-QLD qgip register, 19.7% topic-tagged; '
  'NOT a justice figure. justice_total/justice_count are deprecated dual-names, dropped in step 2. '
  'Money filters applied 2026-08-16. See thoughts/shared/data-map/justice-funding-filter-audit.md';
COMMENT ON COLUMN public.mv_entity_total_funding.recorded_grants_total IS
  'Grant dollars from justice_funding after the mandatory filters. Canonical name; justice_total is the deprecated alias.';
COMMENT ON COLUMN public.mv_entity_total_funding.justice_total IS
  'DEPRECATED dual-name for recorded_grants_total. Dropped in migration 20260816100000.';
...
SELECT sub.*,
       sub.justice_dollars           AS recorded_grants_dollars,
       sub.justice_record_count      AS recorded_grants_count,
       sub.distinct_justice_programs AS distinct_grant_programs,
       sub.justice_states            AS recorded_grants_states,
       sub.in_justice_funding        AS in_recorded_grants
FROM (
...
COMMENT ON MATERIALIZED VIEW public.mv_entity_power_index IS
  'Cross-system power concentration. recorded_grants_* columns (aliases of the deprecated '
  'justice_* names) hold money from the justice_funding TABLE — a grants register, not a justice '
  'figure. justice_* dual-names are dropped in step 2 (20260816100000). Money filters applied '
  '2026-08-16 at all four sites. See thoughts/shared/data-map/justice-funding-filter-audit.md';
COMMENT ON COLUMN public.mv_entity_power_index.in_recorded_grants IS
  'Presence in the recorded-grants register after mandatory filters. Canonical; in_justice_funding is the deprecated alias.';
