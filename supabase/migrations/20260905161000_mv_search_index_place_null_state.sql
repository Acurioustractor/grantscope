-- 20260905161000_mv_search_index_place_null_state.sql
-- Follow-up to 20260905160000 within the hour: the first build had 432 council rows with no state, 430 of them duplicates
-- of a stated row (three "Alice Springs"), and a state filter emptied every kind that carries no state. Matviews cannot be
-- altered in place, so this recreates it; registry and register rows from the first migration stand.
BEGIN;
